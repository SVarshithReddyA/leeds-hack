...
const Meeting = () => {
  ...
  const CLIENT_SECRET = process.env.PEXIP_CLIENT_SECRET;
  const CRUD_ADDRESS = process.env.PEXIP_CRUD_ADDRESS;
  ...
  useEffect(() => {
    const existingMeetingId = new URLSearchParams(window.location.search).get("id");
  ...
  const joinMeeting = async () => {
    try {
      ...
      setParticipantSecret(participant.participant_secret);
    } catch (error) {
  ...
  const setupVpaas = async () => {
    ...
    if (!localVideo) {
      ...
      gridRef.current.appendChild(localVideo);
    }
  ...
  return (
    <div>
      <h1>VPaaS Meeting</h1>
      <button onClick={handleStartMeeting}>{meetingId ? "Rejoin Meeting" : "Start New Meeting"}</button>
      <div
        id="grid"
        ref={gridRef}
        style={{
